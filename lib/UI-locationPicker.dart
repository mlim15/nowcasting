...
class LocationPickerScreen extends StatefulWidget {
  ...
  LocationPickerScreenState createState() =>
      new LocationPickerScreenState(this.location);
}
...
class LocationPickerScreenState extends State<LocationPickerScreen>
  ...
  Widget build(BuildContext context) {
    ...
    if (imagery.coordOutOfBounds(this.location.coordinates)) {
      ...
      _mapOptions =
          map.getMapOptions(context, positionChanged: _positionChanged);
    } else {
      // Else send them so we can start centered on the current coordinates of the saved location.
      _mapOptions = map.getMapOptions(context,
          centerLat: this.location.coordinates.latitude,
          centerLon: this.location.coordinates.longitude,
            ...
            MarkerLayerOptions(
              markers: [
                ...
                Marker(
                    point: _markerLoc,
                    builder: (context) {
                      return ux.locMarker(context,
                          markerColor: ux.alertColor,
                          borderColor: ux.alertColor);
                    }),
                // Current saved location (if applicable)
                Marker(
                    point: this.location.coordinates,
                    builder: (context) {
                      return ux.locMarker(context);
                    })
              ],
