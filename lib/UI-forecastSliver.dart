...
class ForecastSliver extends StatelessWidget {
  ...
  Widget _showFailed() {
    return Column(children: [
      ...
      Padding(
          child: Text("Error: Decoding failed. Pull to refresh.",
              style: ux.latoWhite),
  ...
  Widget build(BuildContext context) {
    ...
    TextEditingController _nameTextController =
        new TextEditingController.fromValue(
            TextEditingValue(text: this.location.name));
    ...
    _notifyPressed() async {
      ...
      if (Platform.isAndroid ||
        ...
        // Toggle notify.
        _notify ? _notify = false : _notify = true;
        rebuildCallback();
              ...
              children: <Widget>[
                Padding(
                  ...
                  child: TextFormField(
                    ...
                    decoration:
                        new InputDecoration(labelText: "Latitude (35 to 51)"),
                    keyboardType: TextInputType.number,
                    validator: (newValue) {
                      if (double.tryParse(newValue) == null) {
                        return 'Invalid latitude.';
                      } else if (!(imagery.sw.latitude.toDouble() <=
                              double.parse(newValue)) ||
                          !(double.parse(newValue) <=
                              imagery.ne.latitude.toDouble())) {
                ...
                Padding(
                  ...
                  child: TextFormField(
                    ...
                    decoration: new InputDecoration(
                        labelText: "Longitude (-88.7 to -66.7)"),
                    keyboardType: TextInputType.number,
                    validator: (newValue) {
                      if (double.tryParse(newValue) == null) {
                        return 'Invalid longitude.';
                      } else if (!(imagery.sw.longitude.toDouble() <=
                              double.parse(newValue)) ||
                          !(double.parse(newValue) <=
                              imagery.ne.longitude.toDouble())) {
                        return "Out of service range.";
                      }
                      return null;
                    },
                          ...
                          children: <Widget>[
                            ...
                            RaisedButton(
                                ...
                                onPressed: () async {
                                  ...
                                  if (_chosenLoc != null) {
                                    _latControl.text =
                                        _chosenLoc.latitude.toString();
                                    _lonControl.text =
                                        _chosenLoc.longitude.toString();
                                  }
                                }),
                            Spacer()
                          ],
                ...
                Padding(
                    ...
                    child: Row(
                      children: <Widget>[
                        FlatButton(
                          child: _isEditable ? Text("Cancel") : Text("Close"),
                          onPressed: () {
                        ...
                        Spacer(),
                        RaisedButton(
                          child: _isEditable ? Text("Save") : Text("Update"),
                          color: ux.nowcastingColor,
                          textColor: Colors.white,
                          onPressed: _isEditable
                              ? () {
                                  if (_formKey.currentState.validate()) {
                                    this.location.coordinates = new LatLng(
                                        double.parse(_latControl.text),
                                        double.parse(_lonControl.text));
                                    io.savePlaceData();
                                    rebuildCallback();
                                    Navigator.of(context).pop();
                                  }
                              ...
                              : () async {
                                  ux.showSnackBarIf(
                                      true, ux.updatingLocationSnack, context);
                                  bool _updateSucceeded =
                                      await loc.currentLocation.update();
                                  rebuildCallback();
                                  Navigator.of(context).pop();
                                  _updateSucceeded
                                      ? ux.showSnackBarIf(true,
                                          ux.locationUpdatedSnack, context)
                                      : ux.showSnackBarIf(
                                          true, ux.locationOffSnack, context);
                                },
                        ),
                      ],
                    ))
    ...
    Future<Widget> populateForecast() async {
      ...
      for (int _i = 0; _i <= 8; _i++) {
        _pixelValues
            .add(await imagery.getPixel(imagery.nowcasts[_i], this.location));
      }
              ...
              Container(
                  ...
                  child: Column(children: <Widget>[
                    Container(
                      ...
                      child: imagery.convert(
                          _pixelValues[_i], imagery.NowcastDataType.icon),
                      decoration: BoxDecoration(
                          shape: BoxShape.rectangle,
                          borderRadius: new BorderRadius.circular(8.0),
                          color: imagery.convert(_pixelValues[_i],
                              imagery.NowcastDataType.colorObj)),
                    ),
                    Container(
                      child: Text(
                          imagery.convert(_pixelValues[_i],
                              imagery.NowcastDataType.description),
                          style: ux.latoWhite),
                    ),
                    Text(
                        DateFormat('HH:mm').format(imagery.nowcasts[_i].legend),
                        style: ux.latoWhite),
                    Text(
                        DateFormat('EEE d').format(imagery.nowcasts[_i].legend),
                        style: ux.latoWhite),
                  ]))
          ]));
    }
    ...
    return new Container(
        height: _editing ? ux.sliverHeightExpanded : ux.sliverHeight,
        margin: ux.sliverMargins,
        child: new Stack(
          children: <Widget>[
            new Container(
              decoration: new BoxDecoration(
                color: _editing ? ux.noticeColor : ux.nowcastingColor,
                shape: BoxShape.rectangle,
                borderRadius: new BorderRadius.circular(8.0),
                boxShadow: <BoxShadow>[
                  new BoxShadow(
                          ...
                          Column(
                            children: <Widget>[
                              ...
                              IconButton(
                                ...
                                onPressed: ((this.location
                                            is loc.CurrentLocation) ||
                                        (loc.savedPlaces.first ==
                                            this.location))
                                    ? null // Disable the "up" button if it's first in the list or the current location box
                                    : () {
                                        int _index = loc.savedPlaces
                                            .indexOf(this.location);
                                        loc.SavedLocation _thisLoc =
                                            loc.savedPlaces[_index];
                                        loc.savedPlaces[_index] =
                                            loc.savedPlaces[_index - 1];
                              ...
                              IconButton(
                                ...
                                onPressed: ((this.location
                                            is loc.CurrentLocation) ||
                                        (loc.savedPlaces.last == this.location))
                          ...
                          Expanded(
                              child: Align(
                                  ...
                                  child: Column(children: [
                                    ...
                                    imagery.coordOutOfBounds(
                                            this.location.coordinates)
                                        // If coordinates of the location are out of service range, display a message
                                        ? Container(
                                            margin: EdgeInsets.all(8),
                                            child: Text(
                                                "Tap the pencil icons to edit this entry and add valid coordinates.",
                                                style: ux.latoWhite))
                          ...
                          Expanded(
                              child: Align(
                                  ...
                                  child: Column(children: [
                                    Container(
                                      padding: EdgeInsets.all(8),
                                      child: new Text(this.location.name,
                                          textAlign: TextAlign.left,
                                          style: TextStyle(fontSize: 16)
                                              .merge(ux.latoWhite),
